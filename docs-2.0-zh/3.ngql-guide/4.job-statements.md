...
`SUBMIT JOB BALANCE DATA IN ZONE`语句会启动一个任务在当前图空间中，均衡各 Zone 内 Storage 服务的分片分布，让各 Zone 内 Storage 服务负载均衡。
...
`SUBMIT JOB BALANCE DATA IN ZONE REMOVE`语句会启动一个任务清空当前图空间指定的 Storage 服务。清空 Storage 服务前，需要确保 Zone 内剩余 Storage 服务数量可以满足设置的副本数。例如，如果设置了副本数为 3，那么在执行该命令前，需要确保剩余 Storage 服务数量大于等于 3。
