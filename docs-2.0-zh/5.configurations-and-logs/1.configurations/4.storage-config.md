...
| `port`                  | `9779`           | Storage 服务的 RPC 守护进程监听端口。Storage 服务对外端口为`9779`，对内端口为`9777`、`9778`和`9780`，{{nebula.name}}使用内部端口进行多副本间的交互。<br/>`9777`：drainer 服务占用端口；`9778`：Admin 服务（Storage 接收 Meta 命令的服务）占用的端口；`9780`：Raft 通信端口。|不支持|
...
| 名称                       | 预设值            | 说明                     |是否支持运行时动态修改|
| :--------------------------| :-------------- | :------------------------ |:----------------------- |
|`data_path`                 | `data/storage`  | 数据存储路径，多个路径用英文逗号（,）分隔。一个 RocksDB 实例对应一个分片。 |不支持|
