...
| `port`                  | `9779`           | Storage 服务的 RPC 守护进程监听端口。同时还会使用相邻的`-2`（`9777`）、`-1`（`9778`）和`+1`（`9780`）端口。<br/>`9777`：drainer 服务占用端口。<br/>`9778`：Admin 服务（Storage 接收 Meta 命令的服务）占用的端口。<br/>`9780`：Storage 服务之间的 Raft 通信端口。|不支持|
