...
本文介绍如何在 Linux 环境中部署 LM 及如何在悦数图数据库及附属软件中配置 LM。有关如何在 K8s 集群中部署 LM，请参见[部署 LM](../../k8s-operator/2.get-started/2.2.deploy-lm.md)。
...
- 使用 Prometheus
  ...
  !!! note
        ...
        默认情况下，LM 使用`9119`端口。如果需要修改端口号，可在上文的配置文件中修改`Port`字段的值，或者修改[在 K8s 中部署 LM](../../k8s-operator/2.get-started/2.2.deploy-lm.md) YAML 文件中的`port`值。
