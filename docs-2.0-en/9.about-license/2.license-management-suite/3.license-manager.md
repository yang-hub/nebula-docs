...
- When deploying the cluster using Kubectl, configure the address and port of the LM through the `spec.metad.licenseManagerURL` field in the cluster configuration file. 

- When deploying the cluster using Helm, specify the address and port of the LM with `--set nebula.metad.licenseManagerURL`.

For more details, see [Install NebulaGraph clusters](../../k8s-operator/4.cluster-administration/4.1.installation/4.1.1.cluster-install.md).
