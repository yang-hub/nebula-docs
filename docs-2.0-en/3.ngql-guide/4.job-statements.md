...
`SUBMIT JOB BALANCE DATA IN ZONE REMOVE` statement starts a job to clear the partitions on specified Storage services in zones in the current graph space. It returns the job ID. Before clearing the Storage services, make sure that the remaining Storage services in zones can meet the set number of replicas. For example, if the number of replicas is set to 3, make sure that the remaining Storage services are greater than or equal to 3 before executing this command.
...
```ngql
# Clear the partitions on the specified Storage services. 
